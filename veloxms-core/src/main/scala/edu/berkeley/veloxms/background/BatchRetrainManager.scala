...
class BatchRetrainManager[T](
  ...
  def batchTrain(): String = {
    if (model.numFeatures == 0) {
      logError("No feature models registed, aborting batch train")
      "Failure: No feature models to train"
    } else {
      ...
      if (lockAcquired) {
        try {
          ...
          val itemFeatures = model.trainAllFeatureModels(trainingData, nextVersion)
          ...
          val avgUser = model.retrainAvgUserInSpark(itemFeatures, observations)

          sparkContext.parallelize(List(avgUser))
            .saveAsTextFile(s"$sparkDataLocation/${newModelLocation.userWeightsLoc}/avg_user")
